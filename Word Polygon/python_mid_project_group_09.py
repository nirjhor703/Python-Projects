...
def load_wordlist():
    with open("wordlist.txt", "r") as f:
        lines = f.readlines()
    return [word.strip().lower() for word in lines if len(word.strip()) >= 3]
...
def update_timer():
    ...
    if time_left > 0:
        ...
        messagebox.showinfo("Time's Up!", f"Score: {len(found_words)}\nWords You Could Have Made:\n\n" + "\n".join(possible))
...
def draw_hexagons():
    ...
    offsets = [
        (0, 0),  # Center
        (0, -hex_size * math.sqrt(3)),
        (-hex_size * 1.5, -hex_size * math.sqrt(3) / 2),
        (hex_size * 1.5, -hex_size * math.sqrt(3) / 2),
        (0, hex_size * math.sqrt(3)),
        (-hex_size * 1.5, hex_size * math.sqrt(3) / 2),
        (hex_size * 1.5, hex_size * math.sqrt(3) / 2)
...
canvas = tk.Canvas(root, width=500, height=250, bg="#263238")
canvas.pack(pady=10)
